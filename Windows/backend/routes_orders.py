...
def _route_stages(route):
    """Стадии детали из маршрута: Л->laser, Г->bender, Св->welder, М->fitter; «Т/Труборез» игнорируется."""
    if not route:
        return []
    stages = []
    seen = set()
...
def _has_untracked_tokens(route):
    ...
    Токены «Т/Труборез» игнорируются: блока «Труборез» больше нет."""
    if not route:
        return False
    for tok in _ROUTE_TOKEN_RE.split(str(route).upper()):
        tok = tok.strip()
...
def _order_progress(db: Session, order: SpecOrder):
    ...
    for i in bend_items:
        done_bend += _stage_done(i, "bender")
...
def _item_response(item: OrderItem, claim=None) -> dict:
    ...
    stage_list = _route_stages(item.route)
    if stage_list:
        stage_done = (not _has_untracked_tokens(item.route)) and _open_stage(item) is None
    else:
        stage_done = item.quantity > 0 and done >= item.quantity
    return {
        "id": item.id,
