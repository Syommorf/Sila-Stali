...
class App:
    ...
    def check_updates(self, manual=False):
        if not getattr(sys, "frozen", False):
            if manual:
                messagebox.showinfo(
                    "Обновление",
                    "Автообновление работает только в собранном .exe.",
                    parent=self.root)
            return
        if manual:
            self._update_prompted = False
        ...
        def worker():
            try:
                info = updater.fetch_latest(api_mod.BASE_URL)
            except Exception:
                return
            ...
            if not latest or not url or not updater.is_newer(latest, VERSION):
                return
            try:
    ...
    def _prompt_update(self, latest, url):
        ...
        if messagebox.askyesno(
                "Доступно обновление",
                f"Доступна новая версия {latest} (у вас {VERSION}).\n\n"
                "Обновить сейчас? Приложение перезапустится.",
                parent=self.root):
            self._do_update(url, latest)

    def _do_update(self, url, latest):
        ...
        try:
            ...
            pass
        tk.Label(win, text=f"Загрузка версии {latest}…", bg=BG, fg=TEXT,
                 font=(FONT, 13, "bold")).pack(pady=(20, 8))
        bar = ttk.Progressbar(win, mode="indeterminate", length=360)
        bar.pack(pady=6)
        bar.start(12)
        status = tk.Label(win, text="Подготовка…", bg=BG, fg=TEXT_SEC,
                          font=(FONT, 10))
        status.pack(pady=(2, 0))
        dest = os.path.join(auth_store.APP_DIR, "updates", "SilaStaliWindows_new.exe")
        box = {}
            ...
            def apply():
                if total:
                    try:
                        ...
                        pass
                    status.config(text=f"{done // 1024} КБ из {total // 1024} КБ")
                else:
                    status.config(text=f"{done // 1024} КБ")
            try:
        ...
        def worker():
            try:
                updater.download(url, dest, on_progress=on_progress)
                box["ok"] = True
            except Exception as e:
                box["err"] = str(e)
            try:
        ...
        def finish():
            ...
            if box.get("ok"):
                try:
                    updater.schedule_update(sys.executable, dest)
                except Exception as e:
                    try:
                        win.destroy()
                    except Exception:
    ...
    def show_login(self):
        ...
        tk.Label(frame, text=f"Версия {VERSION}", bg=BG, fg=TEXT_SEC,
                 font=(FONT, 10)).pack(pady=(0, 2))
        tk.Button(frame, text="Проверить обновления", bg=BG, fg=ACCENT,
                  font=(FONT, 9, "underline"), relief="flat", bd=0,
                  cursor="hand2", activebackground=BG,
                  command=lambda: self.check_updates(manual=True)).pack(pady=(0, 14))
    ...
    def _stage_remaining(self, item, stage):
        st = (item.get("stages") or {}).get(stage)
        if st:
            return max(0, st.get("total", 0) - st.get("done", 0))
        return max(0, item.get("remaining", 0))
    ...
    def _render_order_detail(self, inner, order, users, l_title, l_progress,
        ...
        for item in order.get("items", []):
            done = (item.get("stage_done")
                    if "stage_done" in item
                    else item.get("quantity", 0) > 0 and item.get("remaining", 0) <= 0)
                ...
                if is_manager:
                    _small_btn(acts, "📌 Отметить стадию", "#00695C",
                               lambda it=item: self._stage_dialog(
                                   order["order_id"], it, refresh, executors=benders)
                               ).pack(side="right", padx=(0, 6))
                elif my_block == BLOCK_BENDER:
                    if BLOCK_BENDER in pending:
                        _small_btn(acts,
                                   f"📌 Отметить (осталось {self._stage_remaining(item, BLOCK_BENDER)})",
                                   "#00695C",
                                   lambda it=item: self._mark_dialog(
                                       order["order_id"], it, benders, refresh)
                                   ).pack(side="right", padx=(0, 6))
    ...
    def _mark_dialog(self, order_id, item, benders, refresh):
        ...
        body = tk.Frame(dlg, bg=BG, padx=20, pady=16)
        body.pack(fill="both", expand=True)
        bend_rem = self._stage_remaining(item, "bender")
        tk.Label(body, text=f"{item.get('part_number')} · осталось {bend_rem}",
        ...
        tk.Label(body, text=f"Сколько согнуто (осталось {bend_rem}):",
    ...
    def _stage_dialog(self, order_id, item, refresh, only=None, executors=None):
        ...
        executor_var = None
        if executors:
            word = {"laser": "лазерщик", "bender": "гибщик", "fitter": "слесарь",
                    "welder": "сварщик", "storekeeper": "кладовщик"}.get(stages[0], "")
            tk.Label(body,
                     text=(f"Исполнитель (фактический {word}):" if word
                           else "Исполнитель (фактический):"),
                     bg=BG, fg=TEXT, font=(FONT, 11)).pack(anchor="w", pady=(10, 2))
            enames = [f"{e.get('full_name')} ({e.get('username')})" for e in executors]
            claim_wid = item.get("claim", {}).get("worker_id")
            default_name = enames[0]
            if claim_wid is not None:
                for e in executors:
                    if e.get("id") == claim_wid:
                        default_name = f"{e.get('full_name')} ({e.get('username')})"
                        break
            executor_var = tk.StringVar(value=default_name)
            combo = ttk.Combobox(body, textvariable=executor_var, values=enames,
                                 state="readonly", font=(FONT, 12))
            combo.pack(fill="x")
        ...
        def save():
            ...
            executor_id = None
            if executor_var is not None:
                idx = enames.index(executor_var.get())
                executor_id = executors[idx]["id"]
            self._exec(
                lambda: self.api.complete_item(order_id, item["id"], qty, executor_id, stage=code),
                ok=lambda _: (dlg.destroy(), refresh()))
...
def main():
    ...
    app.show_start()
    root.after(1500, app.check_updates)
    root.mainloop()
