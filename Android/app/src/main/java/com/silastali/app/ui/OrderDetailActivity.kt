...
class OrderDetailActivity : AppCompatActivity() {
    ...
    private fun stageRemaining(item: OrderItemDetail, stage: String): Int {
        val s = item.stages[stage]
        return if (s != null && s.total > 0) maxOf(0, s.total - s.done) else item.remaining
    }

    private var orderId: Int = 0
    private var employeesCache: List<UserItem>? = null
    private val workerRoles = setOf("apprentice", "bender", "senior_bender")
    private val allWorkerRoles = setOf(
        "apprentice", "bender", "senior_bender", "laser_cutter", "fitter", "welder", "storekeeper",
    )

    private fun workerExecutors(): List<UserItem> =
        (employeesCache ?: emptyList()).filter { it.role in allWorkerRoles }

    private fun executorSpinner(executors: List<UserItem>, claimWorkerId: Int?): android.widget.Spinner {
        val sp = android.widget.Spinner(this)
        val labels = executors.map { "${it.full_name.ifEmpty { it.username }} — ${RoleNames.displayName(it.role)}" }
        sp.adapter = android.widget.ArrayAdapter(
            this, android.R.layout.simple_spinner_dropdown_item, labels,
        )
        val idx = claimWorkerId?.let { cw -> executors.indexOfFirst { it.id == cw } } ?: 0
    ...
    private fun handleItemAction(detail: OrderDetail, item: OrderItemDetail) {
        ...
        if (stageRemaining(item, Stages.BENDER) <= 0) {
            showHistoryDialog(item)
            return
        }
    ...
    private fun showItemDialog(detail: OrderDetail, item: OrderItemDetail) {
        ...
        val tvPart = TextView(this).apply {
            text = "${item.part_number} · осталось ${stageRemaining(item, Stages.BENDER)}"
            textSize = 15f
            setPadding(0, 0, 0, 12)
        }
        ...
        val etQuantity = EditText(this).apply {
            hint = "Сколько согнуто (осталось: ${stageRemaining(item, Stages.BENDER)})"
        ...
        if (isManager) {
            val executors = workerExecutors()
            if (executors.isNotEmpty()) {
                container.addView(TextView(this).apply {
                    text = "Исполнитель (фактический гибщик):"
                    textSize = 13f
                    setPadding(0, 8, 0, 4)
                })
                val sp = executorSpinner(executors, item.claim?.worker_id)
                container.addView(sp)
                executorId = { executors[sp.selectedItemPosition].id }
            }
        }
        ...
        val dialog = AlertDialog.Builder(this)
            ...
            .setPositiveButton("Отметить") { _, _ ->
                ...
                complete(detail.order_id, item.id, qty, if (executorId != null) executorId!!() else null)
            }
            .setNegativeButton("Отмена", null)
            .show()
    }
    ...
    private fun showStageMarkDialog(detail: OrderDetail, item: OrderItemDetail, onlyStage: String? = null) {
        ...
        if (isManager) {
            val executors = workerExecutors()
            if (executors.isNotEmpty()) {
                val stageCode = onlyStage ?: pendingStages[0]
                val word = mapOf(
                    Stages.LASER to "лазерщик", Stages.BENDER to "гибщик",
                    Stages.FITTER to "слесарь", Stages.WELDER to "сварщик",
                    Stages.STOREKEEPER to "кладовщик",
                )[stageCode] ?: ""
                container.addView(TextView(this).apply {
                    text = if (word.isNotEmpty()) "Исполнитель (фактический $word):" else "Исполнитель (фактический):"
                    textSize = 13f
                    setPadding(0, 8, 0, 4)
                })
                val sp = executorSpinner(executors, item.claim?.worker_id)
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Отметить") { _, _ ->
                ...
                complete(detail.order_id, item.id, qty, if (executorId != null) executorId!!() else null, stage)
            }
            .setNegativeButton("Отмена", null)
            .show()
    }
    ...
    private fun confirmClose(detail: OrderDetail) {
        val remaining = detail.items.filter { it.needs_bending }.sumOf { stageRemaining(it, Stages.BENDER).toLong() }
        AlertDialog.Builder(this)
