...
class OrderItemAdapter(
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        holder.binding.apply {
            ...
            val done = item.quantity > 0 && item.done_quantity >= item.quantity
            ...
            tvProgress.text = if (done) "✅ ГОТОВО · ${item.quantity} из ${item.quantity}" else "Выполнено: ${item.done_quantity} · Осталось: ${item.remaining}"
            ...
            if (done || isClosed || item.remaining <= 0) {
